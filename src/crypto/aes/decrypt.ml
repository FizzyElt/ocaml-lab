...
let decrypt_block (block : bytes) (key : bytes) : bytes =
    let round_key_of_words = key_expansion key |> round_key_of_words in

    let state = ref (add_round_key block ~round_key:(round_key_of_words 10)) in
    ...
    |> inv_shift_rows
    |> inv_sub_bytes
    |> add_round_key ~round_key:(round_key_of_words 0)
;;
...
let decrypt_cbc (ct : bytes) (key : bytes) (iv : bytes) : bytes =
    ...
    for i = 0 to (Bytes.length ct / 16) - 1 do
      ...
      let p = decrypt_block block key |> xor_bytes !prev in
...
let%test "decrypt_block" =
    ...
    let dt = decrypt_block exp key in
...
let%test "decrypt_cbc: pt.length mod 16 = 0" =
    ...
    in
    let dt = decrypt_cbc ct key iv in
    let pt =
        Hex.to_bytes
